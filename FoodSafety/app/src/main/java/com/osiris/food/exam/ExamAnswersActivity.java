            ...
            public void requestSuccess(int tag, String successResult) {
                ...
                if (json.get("code").getAsInt() == 200) {
                    ...
                    if (answered) {
                        ...
                        int rightCount = 0;
                        for (ExamLocal u : userRealmResults) {
                            LogUtils.d("zkf start for");
                            for (ExamDetail.DataBean.QuestionItemsBean questionItemsBean : dataList) {
                                if (u.getAnswerId() == questionItemsBean.getId()) {
                                    if (answered && examLocalScore == 0) {
                                        LogUtils.d("zkf answer: kaishipanduan");
                                        LogUtils.d("zkf averageScore:" + averageScore);
                                        LogUtils.d("zkf u.getAnswer():" + u.getAnswer() + " questionItemsBean.getAnswer():" + questionItemsBean.getAnswer());
                                        if (u.getAnswer().equals(questionItemsBean.getAnswer())) {
                                            rightCount++;
                                        }
                                    }
                                }
